(function ($) {
    ...
    $('#userlocation').selectpicker().ajaxSelectPicker({
        ajax: {
            url: window.APP_URL + '/api/developments-and-projects',
            type: 'GET',
            dataType: 'json',
            data: function () {
                return {
        ...
        locale: {searchPlaceholder: 'Min. 3 characters required'},
    ...
    $('#userlocation').on('changed.bs.select', function (e, clickedIndex, isSelected, previousValue) {
        if (e.target.value){
            $('#location_ref').val(e.target.selectedOptions[0].className);
        }
    });
    ...
    // Dropdown
    $('.dropdown-menu .more_link').on("click.bs.dropdown", function (e) {
        e.stopPropagation();
        e.preventDefault();
    });

    $('.dropdown-menu .link').on("click.bs.dropdown", function (e) {
        e.stopPropagation();
        e.preventDefault();
    });
    ...
    if ($('.feature_property_slider_sale').length) {
        $('.feature_property_slider_sale').owlCarousel({
            loop: true,
            margin: 30,
            dots: true,
            nav: false,
    ...
    // Initialize Owl Carousel for the first visible tab
    $(".tab-pane.active .fp_single_item_slider").each(function () {
        initOwlCarousel($(this));
    });

    // Reinitialize when a new tab is shown
    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        let targetTab = $($(e.target).attr("href"));
        let slider = targetTab.find(".fp_single_item_slider");
    ...
    if ($('.fp_single_item_slider_inner').length) {
        $('.fp_single_item_slider_inner').owlCarousel({
            loop: false,
            margin: 15,
            dots: false,
            nav: true,
